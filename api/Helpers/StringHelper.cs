...
namespace api.Helpers
{
    public static class StringHelper
    {
        ...
        {
            var regex = new System.Text.RegularExpressions.Regex(@"^([A-Z]{1,2}\d{6,8}|\d{12})$");
            return regex.IsMatch(licenseNumber);
        }

        public static bool IsValidSocialInsuranceNumber(string socialInsuranceNumber)
        {
            var regex = new System.Text.RegularExpressions.Regex(@"^\d{10}$");
            return regex.IsMatch(socialInsuranceNumber);
        }
        ...
        {
            return $"{DateTime.Today:yyMMdd}{sequence:D4}";
        }
    }
