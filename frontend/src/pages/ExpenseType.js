...
import ExpenseTypeTable from '../Table/ExpenseTypeTable'; // THAY ĐỔI
import '../styles/css/ExpenseType.css'; // THAY ĐỔI
import ExpenseTypeFormPopup from '../Table/ExpenseTypeFormPopup'; // THAY ĐỔI
...
export default function ExpenseType() {
    ...
    const apiUrl = `${API_URL}/expense-type`; 
    ...
    useEffect(() => {
        ...
        if (!canView(PERMISSION_IDS.EXPENSE_TYPE)) {
            console.warn(
                `Người dùng không có quyền xem trang Expense Type (ID: ${PERMISSION_IDS.EXPENSE_TYPE}). Đang chuyển hướng...`,
            );
    ...
    return (
        <div className="container-fluid pt-4 px-4">
            <button
                ...
                id="btn_expense_type_filter" // THAY ĐỔI
            ...
            <div className="col-sm-12 col-xl-12 py-4">
                <div className="bg-light rounded h-100 p-4">
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h6 className="mb-4">Loại chi phí</h6>
                        {/* THAY ĐỔI */}
                        <button
                            type="button"
                            className="btn btn-primary"
                            id="btn_add_expense_type" // THAY ĐỔI
                            onClick={handleAdd}
                        >
                            <i className="fa fa-plus me-2"></i>Thêm mới
                        </button>
                    </div>
                    <ExpenseTypeTable // THAY ĐỔI
                        apiUrl={apiUrl}
                        token={token}
                        onEdit={handleEdit}
