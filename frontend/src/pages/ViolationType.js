...
import ViolationTypeTable from '../Table/ViolationTypeTable'; // THAY ĐỔI
import '../styles/css/ViolationType.css'; // THAY ĐỔI
import ViolationTypeFormPopup from '../Table/ViolationTypeFormPopup'; // THAY ĐỔI
...
export default function ViolationType() {
    // THAY ĐỔI
    const [showFilter, setShowFilter] = useState(false);
    const toggleFilter = () => setShowFilter(!showFilter);

    const apiUrl = `${API_URL}/violation-type`; 
    ...
    useEffect(() => {
        ...
        if (!canView(PERMISSION_IDS.VIOLATION_TYPE)) {
            console.warn(
                `Người dùng không có quyền xem trang Violation Type (ID: ${PERMISSION_IDS.VIOLATION_TYPE}). Đang chuyển hướng...`,
            );
    ...
    return (
        <div className="container-fluid pt-4 px-4">
            <button
                ...
                id="btn_violation_type_filter" // THAY ĐỔI
            ...
            <div className="col-sm-12 col-xl-12 py-4">
                <div className="bg-light rounded h-100 p-4">
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <h6 className="mb-4">Loại vi phạm</h6>
                        {/* THAY ĐỔI */}
                        <button
                            type="button"
                            className="btn btn-primary"
                            id="btn_add_violation_type" // THAY ĐỔI
                            onClick={handleAdd}
                        >
                            <i className="fa fa-plus me-2"></i>Thêm mới
                        </button>
                    </div>
                    <ViolationTypeTable // THAY ĐỔI
                        apiUrl={apiUrl}
                        token={token}
                        onEdit={handleEdit}
            ...
            {/* Popup nhập liệu */}
            {showPopup && (
                <ViolationTypeFormPopup // THAY ĐỔI
                    item={editingItem}
