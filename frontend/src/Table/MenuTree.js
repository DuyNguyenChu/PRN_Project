                ...
                <li key={item.id} className="menu-tree-item border-start ps-3 py-2">
                    <div className="d-flex justify-content-between align-items-center">
                        ...
                        <div className="menu-item-actions">
                            {allowUpdate ? ( // Nếu có quyền Update -> Nút Sửa
                                <button
                                    className="btn btn-sm btn-light-primary btn-icon me-1"
                                    title="Sửa"
                                    onClick={() => onEdit(item, false)} // false = not readOnly
                                >
                                    <svg
                                ...
                                // Nếu không có quyền Update -> Nút Chi tiết
                                <button
                                    className="btn btn-sm btn-light-info btn-icon me-1"
                                    title="Chi tiết"
                                    onClick={() => onEdit(item, true)} // true = readOnly
                                >
                                    {/* Icon xem chi tiết (ví dụ: con mắt) */}
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        width="16"
                                        height="16"
                            ...
                            {allowDelete && ( // Chỉ render nếu có quyền Delete
                                <button
                    ...
                    <MenuTree menus={menus} parentId={item.id} onEdit={onEdit} onDelete={onDelete} allowUpdate={allowUpdate} // Truyền quyền xuống
