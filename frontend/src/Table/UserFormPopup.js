...
function UserFormPopup({ item, onClose, apiUrl, token, onSuccess, showConfirmModal, showNotifyModal }) {
    ...
    // (MỚI) State loading user data khi update
    const [loadingUserData, setLoadingUserData] = useState(false);
    // (MỚI) State lưu lỗi khi fetch user data
        ...
        if (isUpdate && item?.id && token) {
            const fetchUserData = async () => {
                ...
                try {
                    ...
                    if (userData) {
                        // Dùng setValues để cập nhật form
                        setValues({
                            ...
                            // API trả về gender là number, giữ nguyên hoặc chuyển string nếu select cần
                            gender: userData.gender !== undefined ? userData.gender : 1,
                            // API trả về userStatus là object, lấy ID
                            userStatus: userData.userStatus?.id || '',
                            // API trả về roles là mảng object, lấy mảng ID
                            roles: userData.roles?.map(role => role.id) || [],
                            // Không cần password khi update
                        });
                    } else {
                        throw new Error("Không tìm thấy dữ liệu người dùng.");
                    }
                } catch (err) {
                    setFetchError('Lỗi tải thông tin người dùng: ' + (err.response?.data?.message || err.message));
                    // Có thể reset form về trạng thái rỗng nếu lỗi nặng
            ...
            fetchUserData();
             ...
             // Nếu là Thêm mới, đảm bảo form dùng initialState (hoặc reset nếu cần)
             setValues(initialState);
        }
        // Thêm setValues vào dependencies
    ...
    if (fetchError) {
        return (
            <div className="popup-overlay"><div className="popup-content p-4 rounded shadow bg-white">
