...
function TripRequestStatusFormPopup({ item, onClose, apiUrl, token, onSuccess, showConfirmModal, showNotifyModal }) {
    const initialState = {
        name: item?.name || '',
    ...
    const handleSubmit = () => {
        ...
        showConfirmModal(item ? 'Bạn có chắc chắn muốn cập nhật?' : 'Bạn có chắc chắn muốn thêm mới?', async () => {
            try {
                // Dùng `values` từ hook
                const payload = { name: values.name, color: values.color , description: values.description};
                if (item) {
...
export default TripRequestStatusFormPopup;
