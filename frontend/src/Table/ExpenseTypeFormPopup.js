...
function ExpenseTypeFormPopup({ item, onClose, apiUrl, token, onSuccess, showConfirmModal, showNotifyModal }) {
    // THAY ĐỔI
    const initialState = {
        name: item?.name || '',
        description: item?.description || '', // GIỮ LẠI
        // BỎ COLOR
    };
    ...
    return (
        <div className="popup-overlay">
            <div className="popup-content p-4 rounded shadow bg-white">
                <h5>{item ? 'Cập nhật loại chi phí' : 'Thêm mới loại chi phí'}</h5>
                {/* THAY ĐỔI TEXT */}
...
export default ExpenseTypeFormPopup;